...
## Follow-up 0.1.21 — slower hover cursor

The first playtest of 0.1.20 reported that the hover weapon animation reads too fast, so the cursor now advances one frame every 200ms and loops once per second instead of 100ms and 500ms. Frame order, hotspot, canvas, rotation tuples and mirroring are unchanged; only `PetView.CURSOR_FRAME_SECONDS` changed.

The timing regression now pins 0/200/400/600/800/1000ms boundaries and the 200ms step, and failed on the 0.1.20 code before the change. Core, desktop, view, female-idle, attack-motion, real-window app, full build and clean-extraction startup checks pass. Version 0.1.21.0 was released as `v0.1.21` (tag on the pull request #22 merge commit) with `dist/DesktopPet-Windows-x64-0.1.21.zip`; released ZIP SHA256: `4FF7D8696CC2BA4261BE6F2C8AFA0642974503E68FBF8DB013C18359187E0BCF`. The unreleased local 0.1.20 artifact is superseded and was removed from `dist/`. The pre-merge verification build hashed `B849458522B94791122BB4F2AAE2C9A65F2F0725DBC5CCC0E2DBBAC0C7A78744`; the release rebuild is not byte-identical, so only the released hash above identifies the shipped archive.
